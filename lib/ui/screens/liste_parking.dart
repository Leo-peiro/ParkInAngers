...
class _ListeParkingState extends State<ListeParking> {
  ...
  Future<void> _getDeviceLocation() async {
    try {
      ...
      parkingsFiltres = trieParkingsParDistance(parkings);
      setState(() {
        build(context);
      });
    } catch (e) {
          ...
          Container(
            ...
            child: ElevatedButton(
              onPressed: () async {
                ...
                if (result != null) {
                  filtrerParkingsCheckbox(result);
    ...
    return ListTile(
      ...
      subtitle: Column(
        children: [
          Row(
            children: [
              const Icon(Icons.local_parking_sharp,color: Colors.lightBlueAccent, size: 24),
          ...
          Row(
            children: [
              ...
              Text(latitude!=0?'Se situe à  ${(calculeDistance(latitude, longitude, parking.latitudeY!.toDouble(),parking.longitudeX!.toDouble())).toStringAsFixed(3)}km de vous':'Problème de localisation', style: const TextStyle(fontStyle: FontStyle.italic,)),
            ],
          ),
        ],
      ),
  ...
  void filtrerParkingsCheckbox(Map<String, bool> result) {
